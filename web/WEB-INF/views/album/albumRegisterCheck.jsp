...
<div class="container" style="padding:50px;">
    <c:if test="${empty error}"><h3>Album Registered</h3></c:if>
    <c:if test="${not empty error}">
        <h3>Album Register failed</h3>
        <h4>${error}</h4>
    </c:if>
    <ul>
        <li>이름: ${album.title}</li>
        <li>아티스트: ${album.artist}</li>
    </ul>
    <a class="no_linkdecoration" href="${pageContext.request.contextPath}/album/">Go to artist list</a>
</div>
